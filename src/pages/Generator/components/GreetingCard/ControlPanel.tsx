    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-cream backdrop-blur-xl border border-cream-dark rounded-[2rem] p-8 min-h-[600px] flex flex-col group transition-all hover:bg-white/60">
                ...
                >
                    {props.userImage ? (
                        ...
                        <div className="space-y-2 text-center">
                            ...
                            <p className="text-bronze-light font-bold">{t('generator.greetingCard.upload.title')}</p>
                            <p className="text-xs text-bronze-light/70">{t('generator.greetingCard.upload.hint')}</p>
                        </div>
                    )}
                    ...
                    {props.isProcessingUploadBg && (
                        <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center z-10 text-primary animate-in fade-in">
                            ...
                            <span className="font-bold animate-pulse">{t('generator.greetingCard.upload.removingBg')}</span>
                        </div>
                    )}
                    ...
                    {props.userImage && !props.isProcessingUploadBg && (
                        <button
                            ...
                            title={t('generator.greetingCard.upload.removeBgTooltip')}
                        >
                            <Scissors size={16} />
                        </button>
                    )}
                ...
                >
                    <FolderHeart size={16} />
                    {t('generator.greetingCard.upload.fromGallery')}
                </button>
                ...
                >
                    ...
                    <span>{t('generator.greetingCard.upload.autoRemoveBg')}</span>
                </div>
            </div>
            ...
            <div className="bg-cream backdrop-blur-xl border border-cream-dark rounded-[2rem] p-6 shadow-sm">
                <h2 className="text-sm font-black text-bronze-light uppercase tracking-widest flex items-center gap-2 mb-4">
                    <Settings size={16} /> {t('generator.greetingCard.settings.title')}
                </h2>

                <div className="space-y-4">
                    {/* Festival Selection */}
                    <div>
                        <label className="text-xs font-bold text-bronze-light ml-2 mb-1 block">{t('generator.greetingCard.settings.festival')}</label>
                        <select
                    ...
                    {/* Style Selection */}
                    <div>
                        <label className="text-xs font-bold text-bronze-light ml-2 mb-1 block">{t('generator.greetingCard.settings.style')}</label>
                        <select
                            value={props.style}
                    ...
                    <div>
                        <label className="text-xs font-bold text-bronze-light ml-2 mb-2 block">{t('generator.greetingCard.settings.bgColor')}</label>
                    ...
                    <div>
                        <label className="text-xs font-bold text-bronze-light ml-2 mb-1 block">{t('generator.greetingCard.settings.aspectRatio')}</label>
                        <select
                    ...
                    {/* Layout Selection */}
                    <div>
                        <label className="text-xs font-bold text-bronze-light ml-2 mb-1 block">{t('generator.greetingCard.settings.layout')}</label>
                    ...
                    <div className="space-y-4 p-4 bg-cream-light/30 rounded-xl border border-cream-dark/20">
                        <h3 className="text-xs font-black text-bronze uppercase tracking-wider">{t('generator.greetingCard.settings.advanced')}</h3>
                        ...
                        <label className={`flex items-center gap-3 group ${!props.userImage ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
                            ...
                            <div className="flex-1">
                                <div className={`text-sm font-bold transition-colors ${!props.userImage ? 'text-bronze-light' : 'text-bronze-text group-hover:text-primary'}`}>
                                    {t('generator.greetingCard.settings.faceSwap')}
                                </div>
                                <div className="text-xs text-bronze-light">{t('generator.greetingCard.settings.faceSwapHint')}</div>
                            </div>
                        </label>
                        ...
                        <label className={`flex items-center gap-3 group ${!props.userImage ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
                            ...
                            <div className="flex-1">
                                <div className={`text-sm font-bold transition-colors ${!props.userImage ? 'text-bronze-light' : 'text-bronze-text group-hover:text-primary'}`}>
                                    {t('generator.greetingCard.settings.autoExpand')}
                                </div>
                                <div className="text-xs text-bronze-light">{t('generator.greetingCard.settings.autoExpandHint')}</div>
                            </div>
                        </label>

                        <div>
                            <div className="flex justify-between items-center mb-1">
                                <label className="text-xs font-bold text-bronze-light">{t('generator.greetingCard.settings.styleIntensity')}</label>
                            ...
                            <input
                                ...
                                title={t('generator.greetingCard.settings.styleIntensityTooltip')}
                            />
                        </div>

                        <div>
                            <label className="text-xs font-bold text-bronze-light mb-1 block">{t('generator.greetingCard.settings.negativePrompt')}</label>
                            <input
                                type="text"
                                value={props.negativePrompt}
                                onChange={(e) => props.onSetNegativePrompt(e.target.value)}
                                placeholder={t('generator.greetingCard.settings.negativePromptPlaceholder')}
                        ...
                        <label className="flex items-center gap-3 cursor-pointer group pt-2 border-t border-cream-dark/20">
                            ...
                            />
                            <div className="flex-1">
                                <div className="text-sm font-bold text-bronze-text group-hover:text-primary transition-colors">{t('generator.greetingCard.settings.showText')}</div>
                            </div>
                        </label>
                        ...
                        {props.showTextOnCard && (
                            <div className="space-y-3 pt-2">
                                <div className="flex justify-between items-center">
                                    <label className="text-xs font-bold text-bronze-light">{t('generator.greetingCard.settings.fontColor')}</label>
                                    <label className="text-primary text-[10px] cursor-pointer hover:underline flex items-center gap-1">
                                        <Upload size={10} /> {t('generator.greetingCard.settings.uploadFont')}
                    ...
                    <div className="space-y-3">
                        <label className="text-xs font-bold text-bronze-light ml-2 mb-1 block">{t('generator.greetingCard.settings.customPrompt')}</label>
                        <div className="relative">
                            <textarea
                                value={props.customPrompt}
                                onChange={(e) => props.onSetCustomPrompt(e.target.value)}
                                placeholder={t('generator.greetingCard.settings.customPromptPlaceholder')}
                        ...
                        >
                            ...
                            {t('generator.greetingCard.settings.optimizePrompt')}
                        </button>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <label className="flex items-center gap-2 text-xs font-bold text-bronze-text">
                                ...
                                />
                                {t('generator.greetingCard.settings.showRecipient')}
                            </label>
                            <label className="flex items-center gap-2 text-xs font-bold text-bronze-text">
                                <input
                                    type="checkbox"
                                    ...
                                    className="w-4 h-4 rounded border-2 border-primary text-primary"
                                />
                                {t('generator.greetingCard.settings.showSender')}
                            </label>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <input
                                ...
                                placeholder={t('generator.greetingCard.settings.recipientPlaceholder')}
                            ...
                            <input
                                ...
                                placeholder={t('generator.greetingCard.settings.senderPlaceholder')}
                        ...
                        <textarea
                            ...
                            placeholder={t('generator.greetingCard.settings.messagePlaceholder')}
                    ...
                    >
                        ...
                        {props.isGenerating ? t('generator.greetingCard.settings.generating') : t('generator.greetingCard.settings.generate')}
                    </button>

                    {props.isGenerating && (
                        <p className="text-center text-xs text-bronze-light animate-pulse font-bold">
                            {(t('generator.greetingCard.settings.loadingSteps', { returnObjects: true }) as string[])[props.loadingStep]}
                        </p>
                    )}
